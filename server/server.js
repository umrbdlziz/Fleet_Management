...
const restartRos = () => {
  if (rosProcess) {
    ...
    colconBuild.on("close", (code) => {
      ...
      const sourceScript = spawn("bash", [
        "-c",
        "source /home/msf1/rmf_ws/install/setup.bash && ros2 launch rmf_demos_gz_classic office.launch.xml",
      ]);

      sourceScript.stdout.on("data", (data) => {
        console.log(`stdout: ${data}`);
      });

      sourceScript.stderr.on("data", (data) => {
        console.error(`stderr: ${data}`);
      });

      sourceScript.on("close", (code) => {
        console.log(`Source script exited with code ${code}`);
      });
    });
  } else {
